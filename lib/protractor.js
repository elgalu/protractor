...
// Temp wrap WebElement click() to take screen shots before and after the click
...
var originalWebElementClick = webdriver.WebElement.prototype.click;
webdriver.WebElement.prototype.click = function() {
    var matchTrace = new Error("at Click");
    var traceStr = filterStackTrace(matchTrace.stack);

    if (jasmine.getEnv().additionalScreenShots) {
        jasmine.getEnv().additionalScreenShots(traceStr, null, null, 'before-click');
    }
    var promise = originalWebElementClick.apply(this, arguments);
    if (jasmine.getEnv().additionalScreenShots) {
        promise.then(function() {
            jasmine.getEnv().additionalScreenShots(traceStr, null, null, 'after-click');
        });
    }
    return promise;
};
var originalWebElementSendKeys = webdriver.WebElement.prototype.sendKeys;
webdriver.WebElement.prototype.sendKeys = function() {
    var matchTrace = new Error("at Sendkeys");
    var traceStr = filterStackTrace(matchTrace.stack);

    if (jasmine.getEnv().additionalScreenShots) {
        jasmine.getEnv().additionalScreenShots(traceStr, null, null, 'before-sendKeys');
    }
    var promise = originalWebElementSendKeys.apply(this, arguments);
    if (jasmine.getEnv().additionalScreenShots) {
        promise.then(function() {
            jasmine.getEnv().additionalScreenShots(traceStr, null, null, 'after-sendKeys');
        });
    }
    return promise;
};
